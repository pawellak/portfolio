...
class CurriculumVitaeCubit extends BaseCubit<CurriculumVitaeState> {
  ...
  Future<List<ProjectModel>> _getProjectList() async => _repository.getProjectList();
  ...
  Future<pw.Widget> _buildSectionProjects(List<ProjectModel> modelList) async {
    final list = await Future.wait(modelList.map((e) async => ProjectImageModel.fromModel(e)).toList());
    return CvSectionProjects(list);
  }
